...
- [x] 强化 GitHub Actions CI：固定 MoonBit `0.10.3+16975d007`，归档 SHA-256 校验、工具链缓存、依赖更新、全目标检查、测试、格式化、接口生成和合规脚本。
...
- [x] 增加面向用户的 `CHANGELOG.md`，记录版本、功能、兼容性和验收基础设施变化。
- [ ] 本轮尚未推送 GitHub、GitLink 或 Mooncakes；等待用户后续明确发布指令。

## 当前本地审计口径

- 工作分支：`main`（本轮只做本地修改）。
- 工具链目标：MoonBit `0.10.3+16975d007`（`moonc v0.10.3`）。
